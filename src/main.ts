...
class Carousel {
  ...
  private render() {
    ...
    for (let i = this.currentIndex; i < this.currentIndex + 3; i++) {
      ...
      quantityInput.classList.add('numInput', 'bg-transparent', 'text-center', 'text-2xl', '[appearance:textfield]', 'focus:border-transparent', 'focus:outline-none', 'focus:ring-0', '[&::-webkit-inner-spin-button]:appearance-none', '[&::-webkit-outer-spin-button]:appearance-none');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const pizzaAmountInputs = document.querySelectorAll('.pizza-amount-input');

  function displayOrderSummary() {
    const summaryDiv = document.createElement('div');
    summaryDiv.innerHTML = '<h2>Order Summary:</h2>';

    for (const pizzaName in orderDetails.pizzas) {
      const amount = orderDetails.pizzas[pizzaName];
      if (amount > 0) {
        const pizzaSummary = document.createElement('p');
        pizzaSummary.textContent = `${pizzaName}: ${amount}`;
        summaryDiv.appendChild(pizzaSummary);
      }
    }

    yourOrder.innerHTML = '';
    yourOrder.appendChild(summaryDiv);
  }
  ...
  pizzaAmountInputs.forEach((input, index) => {
    const pizzaName = Object.keys(orderDetails.pizzas)[index];
    input.addEventListener('input', () => {
      const inputValue = (input as HTMLInputElement).value;
      orderDetails.pizzas[pizzaName] = parseInt(inputValue);
    });
  });
});
